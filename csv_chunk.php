...
function getCSVReaderFFI(): ?FFI
{
    ...
    try {
        $ffi = FFI::cdef(
            'char *csv_read_chunk(const char *path, long offset, long limit);
 ...
 */
function readCsvChunkFFI(FFI $ffi, string $filePath, int $offset, int $limit): array
{
    // Delega parseo CSV al motor C y transforma la salida JSON a array PHP.
    $ptr = $ffi->csv_read_chunk($filePath, $offset, $limit);
...
function readCsvChunk(string $filePath, int $startOffset, int $limit): array
{
    ...
    $chunk = readCsvChunkFFI($ffi, $filePath, $startOffset, $limit);
    $chunk['engine'] = 'ffi';
    return $chunk;
}
